...
function MultiItem:AddBtn2Item(evt, thing, objs)
	...
	if thing ~= nil and thing.ThingType == g_emThingType.Item then 
		...
		thing:AddBtnData(
			...
			thing:RemoveBtnData("一键幽淬12次");
			thing:AddBtnData(
				"一键幽淬12次", 
				"res/Sprs/ui/icon_hand", 
				"GameMain:GetMod('MultiItem'):YouCuiItems2(bind)", 
				"一建提升物品品质12次，必然成功", 
				nil
			);
...
function MultiItem:MultiItems(item)
	iCount = item.Count;
	if iCount >= 4999 then	--存盘的时候会被修正，和物品的最大堆叠数量取min
		return;
	end
	item:ChangeCount(iCount*2)
end

function MultiItem:YouCuiItems2(item)
		for i=1,12 do
